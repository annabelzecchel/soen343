...
class OpenAIService {
  ...
  final Logger _logger = Logger('OpenAIService');
  late String _apiKey;
...
// PUT THE API KEY HERE
    if (apiKey.isEmpty) {
      if (kDebugMode) {
        _logger.warning('Using fallback key for development');
        apiKey = 'put key here';
      } else {
    ...
    if (!apiKey.startsWith('sk-')) {
      _logger.severe('Invalid OpenAI API key format');
      throw Exception('Invalid API Key format - should start with sk-');
    }

    _apiKey = apiKey;
    _isInitialized = true;
    _logger.info('OpenAIService initialized successfully');
  }
  ...
  Future<Map<String, dynamic>> generateChatResponse(
    ...
    if (!_isInitialized) {
      try {
        ...
        return {
          'success': false,
          'message': 'API key not configured properly: $e'
        };
      }
    }
    ...
    try {
      ...
      _logger.severe('Exception when calling OpenAI API: $e');
      return {
        'success': false,
